...
export function PlatformAuditList({
  ...
  if (events.length === 0) return <p className="text-muted-foreground text-sm">{emptyDescription}</p>

  return (
    <ol className="divide-border/70 divide-y">
      {events.map((event) => (
        <li className="grid gap-1 py-3 text-sm sm:grid-cols-[minmax(0,1fr)_auto] sm:gap-x-6" key={event.id}>
          <div>
