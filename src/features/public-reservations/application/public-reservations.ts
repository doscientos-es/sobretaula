...
function isServiceRow(
  row: PublicReservationProfileRow,
): row is PublicReservationProfileRow & {
  ends_at_time: string;
  service_id: string;
  service_name: string;
...
export const getPublicReservationProfile = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<PublicReservationProfile | null> => {
    ...
    if (error)
      throw new Error(`public_reservation_profile_failed:${error.code}`);
...
export const createPublicReservation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (error) {
      if (
        error.code === "23P01" ||
        error.message.includes("public_slot_unavailable")
      ) {
...
export const getPublicReservation = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<PublicReservation | null> => {
    ...
    if (error)
      throw new Error(`public_reservation_lookup_failed:${error.code}`);
    const row = (rows ?? [])[0] as
      | {
          ends_at: string;
          id: string;
...
export const getPublicReservationAvailability = createServerFn({
  ...
  .handler(async ({ data }): Promise<string[]> => {
    ...
    if (error)
      throw new Error(`public_reservation_availability_failed:${error.code}`);
    return ((rows ?? []) as Array<{ starts_at: string }>).map(
      (row) => row.starts_at,
    );
  });
...
export const cancelPublicReservation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (error)
      throw new Error(`public_reservation_cancel_failed:${error.code}`);
