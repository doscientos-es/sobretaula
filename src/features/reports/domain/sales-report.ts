...
export function summarizeProducts(products: readonly SoldProduct[]): ProductSummary[] {
  ...
  for (const product of products) {
    const current = byName.get(product.name) ?? { name: product.name, quantity: 0, amountCents: 0 }
    byName.set(product.name, {
      name: product.name,
      quantity: current.quantity + product.quantity,
      amountCents: current.amountCents + product.amountCents,
    })
  }
...
export function aggregateSales(
  ...
  ticketCount = new Set(products.map((product) => product.name)).size,
) {
  ...
  const vatCents = products.reduce(
    (sum, product) =>
      sum + Math.round((product.amountCents * product.vatRateBps) / (10_000 + product.vatRateBps)),
    0,
  )
  return {
    grossCents,
    byMethod,
    vatCents,
    ticketAverageCents: ticketCount ? Math.round(grossCents / ticketCount) : 0,
  }
}
