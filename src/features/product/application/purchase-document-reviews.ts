  ...
  .handler(
    ...
    > => {
      ...
      const {
        ...
        .select("id, file_name, status, extraction, object_path, created_at", {
          count: "exact",
        })
      ...
      if (error)
        throw new Error(`purchase_document_reviews_load_failed:${error.code}`);
...
export const applyPurchaseDocumentReview = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (review.status !== "approved")
      throw new Error("purchase_document_must_be_approved");
    ...
    if (noteError || !note) {
      ...
      throw new Error(
        `purchase_document_apply_failed:${noteError?.code ?? "unknown"}`,
      );
    }
