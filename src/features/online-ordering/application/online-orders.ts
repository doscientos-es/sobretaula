...
import { canAdvanceOnlineOrder, type OnlineOrderStatus } from '../domain/order-status'
...
export const listOnlineOrders = createServerFn({ method: 'GET' })
  ...
  .validator(readInput)
  .handler(async ({ context, data }): Promise<PaginatedResult<OnlineOrderSummary>> => {
...
export const updateOnlineOrderStatus = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: order, error: loadError } = await supabase
      ...
      .eq('venue_id', data.venueId)
      .single()
    if (loadError) throw new Error(`online_order_load_failed:${loadError.code}`)
