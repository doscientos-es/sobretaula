...
export const createSubscriptionPayment = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (invoiceError || !invoice)
      throw new Error(`subscription_invoice_create_failed:${invoiceError?.code ?? 'unknown'}`)
    ...
    if (attemptError)
      throw new Error(`subscription_payment_attempt_create_failed:${attemptError.code}`)
