...
export const operationalUrl = (path: string) =>
  `/t/${tenant}/l/${venue}${path}`;

export async function expectHealthyPage(page: Page, label: string) {
  await expect(
    page.locator("body"),
    `${label}: la página no debe quedar vacía`,
  ).not.toBeEmpty();
  await expect(
    page.getByText(
      /Something went wrong|No se ha podido cargar esta pantalla/i,
    ),
    `${label}: no debe mostrar error boundary`,
  ).toHaveCount(0);
}

export async function openOperationalPage(
  page: Page,
  path: string,
  label: string,
) {
  await page.goto(operationalUrl(path), { waitUntil: "domcontentloaded" });
  // Wait for the shell and lazy route to settle before interacting with controls.
