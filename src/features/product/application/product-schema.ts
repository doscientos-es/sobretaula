...
export const restoreRecipeVersionInput = recipeQueryInput.extend({ version: z.number().int().positive() })
export const channelPriceInput = productTenantInput.extend({ menuItemId: z.string().uuid(), channel: z.enum(['room', 'web', 'delivery', 'takeaway']), priceCents: z.number().int().min(0).max(1_000_000) })
export const inventoryQueryInput = productTenantInput.extend({ venueId: z.string().uuid() })
export const inventoryMovementInput = inventoryQueryInput.extend({ ingredientId: z.string().uuid(), kind: z.enum(['purchase', 'sale', 'waste', 'adjustment']), quantity: z.number().max(1_000_000).refine((value) => value !== 0), unitCostCents: z.number().min(0).max(1_000_000).optional(), reason: z.string().trim().min(2).max(200) })
