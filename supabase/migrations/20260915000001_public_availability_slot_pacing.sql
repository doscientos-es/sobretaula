-- Corrige el límite de aforo de la disponibilidad pública: las reservas deben
-- contarse dentro del slot real (por ejemplo, 13:30), no desde la hora en punto.
...
as $$
  with target as (
    select t.id tenant_id, t.timezone, s.id service_id, s.venue_id,
      ...
      coalesce((select value::integer from jsonb_each_text(ar.duration_minutes_by_party)
        ...
        order by key::integer limit 1), 120) duration
    from public.tenants t
    join public.services s on s.tenant_id = t.id and s.is_active
    ...
    select target.*,
      (p_date + target.starts_at_time + slots.n * target.slot_minutes * interval '1 minute')
        at time zone target.timezone starts_at,
      (p_date + target.starts_at_time
        + (slots.n * target.slot_minutes + target.duration) * interval '1 minute')
        at time zone target.timezone ends_at
    from target cross join lateral generate_series(
      0,
      greatest(0, floor(extract(epoch from (target.ends_at_time - target.starts_at_time)) / 60
  ...
  where extract(dow from (c.starts_at at time zone c.timezone))::smallint =
    ...
    and not exists (
      ...
      where cl.tenant_id = c.tenant_id and cl.venue_id = c.venue_id
        and cl.period && tstzrange(c.starts_at, c.ends_at, '[)')
    )
    and (c.max_reservations_per_slot is null or (
      select count(*) from public.reservations r
      where r.tenant_id = c.tenant_id and r.venue_id = c.venue_id
        and r.status in ('pending', 'confirmed', 'seated')
        and r.starts_at >= date_trunc('hour', c.starts_at)
          + floor(extract(minute from c.starts_at) / c.slot_minutes)
            * c.slot_minutes * interval '1 minute'
        and r.starts_at < date_trunc('hour', c.starts_at)
          + (floor(extract(minute from c.starts_at) / c.slot_minutes) + 1)
            * c.slot_minutes * interval '1 minute'
    ) < c.max_reservations_per_slot)
    and (c.max_covers_per_slot is null or (
      select coalesce(sum(r.party_size), 0) from public.reservations r
      where r.tenant_id = c.tenant_id and r.venue_id = c.venue_id
        and r.status in ('pending', 'confirmed', 'seated')
        and r.starts_at >= date_trunc('hour', c.starts_at)
          + floor(extract(minute from c.starts_at) / c.slot_minutes)
    ...
    and exists (
      ...
      where tb.tenant_id = c.tenant_id and tb.venue_id = c.venue_id
        ...
        and not exists (
          ...
          where rt.tenant_id = c.tenant_id and rt.table_id = tb.id
