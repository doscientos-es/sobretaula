...
setup('authenticate E2E roles against Supabase-test', async ({ page }) => {
  ...
  const configured = roles.map((role) => {
    ...
    return { role, email: process.env[`E2E_${key}_EMAIL`] ?? defaultCredentials[role][0], password: process.env[`E2E_${key}_PASSWORD`] ?? defaultCredentials[role][1] }
  })
  setup.skip(!process.env.SUPABASE_TEST_URL, 'Requires SUPABASE_TEST_URL and dedicated E2E accounts')
