...
export const createReservationService = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (data.endsAtTime <= data.startsAtTime)
      throw new Response("Invalid service", { status: 422 });
    const supabase = createRequestSupabaseClient(context.tenantMembership.accessToken);
...
export const createReservation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (assignmentError) {
      ...
      if (assignmentError.code === "23P01")
        throw new Response("Table unavailable", { status: 409 });
