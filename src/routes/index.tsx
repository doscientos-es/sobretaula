...
import { ArrowRight, BarChart3, Building2, Boxes, CalendarDays, ChartNoAxesCombined, CircleDollarSign, ClipboardList, ShoppingBag, Users, Utensils } from 'lucide-react'
...
import { MODULE_DEFINITIONS } from '@/features/tenancy'
...
function TenantPicker() {
  ...
  if ('mode' in data && data.mode === 'landing') return <MarketingLanding />
  const { tenants } = data
...
const marketingModules = [
  ...
  { key: 'analytics', icon: ChartNoAxesCombined, outcome: 'Descubre qué platos y trabajadores son más rentables.' },
  ...
  { key: 'online_ordering', icon: ShoppingBag, outcome: 'Vende online sin cambiar tu forma de trabajar.' },
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_right,var(--color-primary)/12,transparent_38%),var(--color-background)]">
      <header className="mx-auto flex max-w-6xl items-center justify-between px-5 py-5 sm:px-8">
        <Link className="flex items-center gap-2 font-semibold tracking-tight" to="/">
          <span className="st-saas-brand-mark"><Utensils className="size-3.5" /></span>
          SobreTaula
        </Link>
        <div className="flex items-center gap-2">
          <Link className="rounded-lg px-3 py-2 text-sm font-medium" to="/login">Entrar</Link>
          <Link className="bg-primary text-primary-foreground rounded-lg px-3 py-2 text-sm font-medium shadow-sm" to="/registro">Probar SobreTaula</Link>
        </div>
      </header>
      <section className="mx-auto max-w-6xl px-5 pb-14 pt-12 sm:px-8 sm:pt-20">
        <div className="max-w-3xl">
          <Badge variant="neutral">Modular. Sin cambiarlo todo.</Badge>
          <h1 className="mt-5 text-4xl font-semibold tracking-tight sm:text-6xl">Añade solo lo que tu restaurante necesita.</h1>
          <p className="text-muted-foreground mt-5 max-w-2xl text-lg leading-8">Empieza con una operativa sencilla y añade inventario, reservas, estadísticas o fidelización cuando te aporten valor. SobreTaula crece contigo, sin obligarte a sustituir lo que ya funciona.</p>
          <div className="mt-7 flex flex-wrap gap-3">
            <Link className="bg-primary text-primary-foreground inline-flex items-center gap-2 rounded-lg px-4 py-2.5 font-medium shadow-sm" to="/registro">Crea tu restaurante <ArrowRight className="size-4" /></Link>
            <a className="border-border bg-card inline-flex items-center rounded-lg border px-4 py-2.5 font-medium" href="#modulos">Ver módulos</a>
          </div>
        </div>
        <div className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-4" id="modulos">
          {marketingModules.map(({ key, icon: Icon, outcome }) => {
            ...
            return (
              <Card className={key === 'core' ? 'border-primary/40 ring-primary/10 ring-2' : ''} key={key}>
                <CardContent className="pt-6">
                  ...
                  <p className="text-muted-foreground mt-3 text-xs leading-5">{definition.description}</p>
                  <Link className="text-primary mt-4 inline-flex items-center gap-1 text-sm font-medium" to="/registro">Añadir este módulo <ArrowRight className="size-3.5" /></Link>
                </CardContent>
              </Card>
            )
          })}
        ...
        <section className="border-border bg-card mt-14 grid gap-6 rounded-2xl border p-6 sm:grid-cols-[1.2fr_1fr] sm:p-8">
          <div><p className="text-primary text-sm font-semibold">La decisión es tuya</p><h2 className="mt-2 text-2xl font-semibold">No pagues por una transformación que no necesitas.</h2><p className="text-muted-foreground mt-3 leading-7">Puedes conservar tu TPV actual y empezar por un módulo concreto. Si mañana quieres conocer tu margen real o automatizar las reservas, lo activas sin migraciones traumáticas.</p></div>
          <div className="bg-secondary/60 rounded-xl p-5"><p className="font-medium">Ejemplo: Estadística Avanzada</p><p className="text-muted-foreground mt-2 text-sm leading-6">“¿Qué trabajador te sale más rentable?” “¿Qué plato vende mucho pero apenas deja margen?” Respuestas accionables a partir de tus propios datos.</p><Link className="text-primary mt-4 inline-flex items-center gap-1 text-sm font-medium" to="/registro">Descubrir estadísticas <ArrowRight className="size-3.5" /></Link></div>
        </section>
      </section>
    </main>
  )
