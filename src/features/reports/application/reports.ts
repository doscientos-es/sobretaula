...
export const getSalesReport = createServerFn({ method: 'GET' }).middleware([authMiddleware, tenantMembershipMiddleware, operationalTenantMiddleware]).validator(salesReportInput).handler(async ({ context, data }) => {
  if (!['owner', 'manager'].includes(context.tenantMembership.role)) throw new Response('Forbidden', { status: 403 })
  const supabase = createRequestSupabaseClient(context.tenantMembership.accessToken)
  const { data: sessions, error: sessionError } = await supabase.from('table_sessions').select('id, discount_cents').eq('tenant_id', data.tenantId).eq('venue_id', data.venueId)
  ...
  const discountsCents = (sessions ?? []).reduce((sum, session) => sum + Number(session.discount_cents ?? 0), 0)
  if (!sessionIds.length) return { grossCents: 0, byMethod: {}, vatCents: 0, ticketAverageCents: 0, netCollectedCents: 0, refundedCents: 0, discountsCents: 0, products: [], productSummary: [] }
  ...
  if (refunds.error && refunds.error.code !== '42P01') throw new Error(`sales_refunds_failed:${refunds.error.code}`)
  const refundedCents = (refunds.data ?? []).reduce((sum, refund) => sum + (refund.amount_cents as number), 0)
  const aggregate = aggregateSales((payments.data ?? []).map((row) => ({ method: row.method as string, amountCents: row.amount_cents as number })), (items.data ?? []).map((row) => ({ name: row.name_snapshot as string, quantity: row.quantity as number, amountCents: (row.quantity as number) * (row.unit_price_cents as number), vatRateBps: row.vat_rate_bps as number })), sessionIds.length)
  const soldProducts = (items.data ?? []).map((row) => ({ name: row.name_snapshot as string, quantity: row.quantity as number, amountCents: (row.quantity as number) * (row.unit_price_cents as number), vatRateBps: row.vat_rate_bps as number }))
  return { ...aggregate, netCollectedCents: Math.max(0, aggregate.grossCents - refundedCents - discountsCents), refundedCents, discountsCents, products: items.data ?? [], productSummary: summarizeProducts(soldProducts) }
})

export const exportSalesReportCsv = createServerFn({ method: 'GET' }).middleware([authMiddleware, tenantMembershipMiddleware, operationalTenantMiddleware]).validator(salesReportInput).handler(async ({ context, data }) => {
  if (!['owner', 'manager'].includes(context.tenantMembership.role)) throw new Response('Forbidden', { status: 403 })
  const supabase = createRequestSupabaseClient(context.tenantMembership.accessToken)
  const { data: sessions, error } = await supabase.from('table_sessions').select('id').eq('tenant_id', data.tenantId).eq('venue_id', data.venueId)
