...
| F3 · Motor de reservas | Turnos, pacing, disponibilidad, best-fit, EXCLUDE                           | Implementado; las RPC públicas están activas y la creación staff usa `last_operation_id` con índice único; falta humo dedicado de reintento/concurrencia                                                                                                                                                                                |
| F4 · Vista de servicio | Plano en vivo, sentar/mover/unir, walk-ins, espera, no-show                 | Implementado; entrega bloqueada                                                                                                                                                                                                                                                                                                         |
| F5 · Cuenta de mesa    | Catálogo, líneas, dividir, cerrar, cobrar                                   | Implementado en navegador/tablet; división por porcentaje, importe y producto en UI, asignación de líneas a pagos y límites de cantidad en RPC preparados; aplicación de migración y humo integrado pendientes                                                                                                                          |
...
| F8 · Reservas públicas | Reserva sin cuenta, gestión, avisos, espera y ficha de cliente              | Parcial; reserva, disponibilidad, gestión por token, confirmación por email y recordatorio 24 h activos; rate limiting por contacto, condiciones versionadas, espera futura, historial ampliado, etiquetas, alergias y preferencias ya están implementados; falta validación operativa, E2E autenticado y retención/privacidad avanzada |
| F9 · Control horario   | PIN, pausas, jornadas, auditoría y exportación                              | Implementado en navegador; portal personal, terminal compartido online, PIN bcrypt, límites de intentos, eventos inmutables, cola offline personal idempotente, cambios de centro con histórico, informe avanzado por empleado/local y descarga CSV desde la UI; la validación legal formal sigue siendo externa                        |
...
| F11 · Entrega          | Documentación operativa, smoke, despliegue autorizado                       | Parcial; smoke público de navegador verificado, tests locales y runbook de recuperación documentado; E2E autenticado, migración remota y despliegue autorizado pendientes                                                                                                                                                               |
...
El refuerzo `20260910000088_reservation_anti_abuse_terms_guest_attributes.sql`
está aplicado en el proyecto autorizado y verificado. La creación pública usa
una clave hash de contacto con ventana horaria y límite de cinco intentos, sin
exponer IP ni datos sensibles al navegador. Las condiciones publicadas tienen
versión, se muestran antes de reservar y la reserva conserva la versión aceptada.
La lista de espera admite fecha/hora futura, estado de oferta/caducidad y
cancelación sin borrar el histórico. La ficha de cliente combina reservas,
cancelaciones, no-shows, visitas, gasto, notas, etiquetas, alergias y
preferencias estructuradas. Aún falta validar con asesoría la política de
retención, exportación y anonimización de estos datos.
...
### Motor laboral español indicativo (D5.1)

El dominio calcula a partir de los eventos inmutables tramos efectivos, jornadas
partidas, cruce de medianoche, minutos nocturnos por zona horaria, minutos en
festivo configurado, exceso diario y alertas de pausa y descanso entre jornadas.
El exceso se clasifica como extra o complementario según sea jornada completa o
parcial. Los umbrales son parámetros explícitos, con valores iniciales de 12 h
de descanso, pausa de 15 minutos tras más de 6 h continuadas y tramo nocturno
22:00–06:00. Es una ayuda operativa, no una liquidación automática: convenio,
...
### Portal y administración laboral (D5.2b)

El portal personal carga la condición laboral vigente, la zona horaria del tenant y
los festivos del local para mostrar horas trabajadas, nocturnidad, trabajo en
festivo, exceso diario e incidencias de pausas o descanso. El resumen está
identificado como orientativo y no sustituye el convenio ni una nómina.

Owner y manager pueden configurar por empleado el tipo de jornada, objetivo diario,
pausa mínima, descanso entre jornadas y tramo nocturno, además de añadir festivos
del local. Las condiciones se guardan por fecha de vigencia y los festivos por
...
autogestión, avisos, cliente, espera y grupos— está documentado en
[`reservations-completion-plan.md`](./reservations-completion-plan.md); el
antiabuso, las condiciones versionadas y los atributos estructurados de cliente
ya cuentan con implementación, pero siguen pendientes de evidencia operativa.
...
Todo pendiente. Los nuevos ajustes fiscales quedan en `test` y ningún tenant
puede emitir en `prod` mientras no se cierre la checklist completa y un asesor
fiscal valide el reparto de responsabilidad. La pantalla no permite cambiar el
entorno: esto es intencionado hasta disponer del adaptador AEAT y del flujo de
activación explícita.

La configuración cargada conserva el entorno persistido, pero
`canIssueInvoices` solo autoriza `test`; así una fila antigua en `prod` no se
transforma silenciosamente en `test` ni puede emitir por accidente.
...
| Redsys recurrente                                               | Alta inicial preparada; cobro recurrente pendiente           | La alta inicial ya crea factura/intento y genera formulario firmado; el webhook valida HMAC y es idempotente. La terminal prevista es `999`. Falta confirmar con Redsys la habilitación COF/MIT/tokenización, recibir y cifrar `Ds_Merchant_COF_TXNID`, ejecutar renovaciones mediante REST y conectar el cron de cobro. |
| Depósitos de reservas                                           | Modelo, RLS y webhook firmado preparados; checkout pendiente | `reservation_deposits` admite idempotencia y estados, y el callback valida HMAC antes de llamar a la RPC protegida. Falta conectar un proveedor de checkout y su creación de intents; no se deben anunciar depósitos cobrables todavía.                                                                                  |
...
### Recordatorio de reserva 24 h (D6)
...
reserva antes de enviar. La migración se aplicó al único proyecto autorizado como
`reservation_reminders` (versión `20260910223352`) mediante el flujo de
migraciones. Se verificaron el trigger de reservas y las RPC de reclamación y
cancelación; no se insertaron ni consultaron datos operativos.
...
- Las cargas CSV de carta, reservas y clientes usan un control nativo de archivo
  para mantener el drag & drop accesible en teclado y lector de pantalla.
